# -*- coding: utf-8 -*-
# Gaïa Nexus – Dashboard IA Sécheresse V2
# ✅ Utilise les données météo réelles + scaler_v2 + modèle LSTM V2
...
def fetch_meteo_reel(lat, lon, days=30):
    """Récupère les 30 derniers jours de données météo réelles via Open-Meteo."""
    ...
    # Calcul humidité du sol par bilan hydrique (identique à l'entraînement)
    h = 0.5
    humidite = []
...
def build_sequence(df, scaler):
    """
    Construit la séquence d'entrée pour le modèle IA à partir
    des données météo réelles des 30 derniers jours.
    Applique le même scaler que celui utilisé à l'entraînement.
    """
    seq = df[FEATURES].values.copy()           # (30, 5)
    seq = scaler.transform(seq)                 # normalisation identique à l'entraînement
    return seq[np.newaxis, :, :]               # (1, 30, 5)
...
# Sélection commune
commune = st.selectbox("🏘️ Sélectionnez une commune (pilote Var)", list(COMMUNES.keys()))
...
# Chargement des données (existantes ou fetch automatique)
...
try:
    sequence = build_sequence(df_meteo.tail(30), scaler)  # (1, 30, 5)
    predictions     = model.predict(sequence)              # (1, 30, 3)
    preds           = predictions[0]                       # (30, 3)
    ...
    def color_niveau(val):
        colors = {"Faible": "#d4edda", "Modérée": "#fff3cd", "Sévère": "#f8d7da"}
